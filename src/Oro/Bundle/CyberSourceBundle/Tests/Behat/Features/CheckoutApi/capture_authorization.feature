...
Feature: Capture authorization
  ...
  As an Administrator
  I want to have ability to capture order payment authorization transaction
  ...
  Scenario: Failed capture action for payment authorization transaction
    ...
    And I should see following actions for Authorize in grid:
      | Capture |
      | Cancel  |
    When I click "Capture" on row "Authorize" in grid "Order Payment Transaction Grid"
    ...
    Then I should see "Could not capture authorization." flash message

  Scenario: Successful capture action for payment authorization transaction
    ...
    Then I should see following "Order Payment Transaction Grid" grid:
      | Payment Method | Type      | Successful |
      | CyberSource    | Capture   | No         |
      | CyberSource    | Authorize | Yes        |
    And I should see following actions for Authorize in grid:
      | Capture |
      | Cancel  |
    And I should not see following actions for Capture in grid:
      | Capture |
      | Cancel  |
    When I click "Capture" on row "Authorize" in grid "Order Payment Transaction Grid"
    Then I should see "Charge The Customer" in the "UiWindow Title" element
